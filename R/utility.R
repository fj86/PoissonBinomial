check.args.GPB <- function(x, probs, val_p, val_q, wts, method, log.p = FALSE) {
  ...
  if(length(val_p) != n || length(val_q) != n)
    stop("'probs', 'val_p' and 'val_q' must have the same length!")
  ...
  method <- match.arg(
    arg = method,
    choices = c(
      "DivideFFT", "Convolve", "Characteristic", "Normal", "RefinedNormal"
    )
  )
...
transformGPB <- function(x, probs, val_p, val_q, wts) {
  ...
  out <- list(
    ...
    val_p = val_gr,
    val_q = val_lo,
    compl.range = sum_min:sum_max,
    inner.range = sum_min_in:sum_max_in,
    inner.size = sum_max_in - sum_min_in + 1,
    n = np,
    diffs = diffs
  )
